...
from typing import Union, Sequence, Tuple, Dict, Any, final, Callable, Generic, TypeVar, Literal, Type

class Dimension:
    pass

IndexDomainT = TypeVar("IndexDomainT", bound=Tuple[Type[Dimension], ...])

class Index(Generic[IndexDomainT]):
    val: Union[int, Literal[math.inf], Literal[-math.inf]]

    def __init__(self, val: Union[int, Literal[math.inf], Literal[-math.inf]]):
        self.val = val

    def __add__(self, other: int) -> "Index":
        if self.val in (math.inf, -math.inf):
            return self
    ...
    def __repr__(self):
        type_args = [type_arg.__name__ for type_arg in typing_inspect.get_args(typing_inspect.get_args(self.__orig_class__)[0])]
        return f"Index[{', '.join(type_args)}]({self.val})"

    def __class_getitem__(cls, params):
        if not isinstance(params, tuple):
            params = (params,)

        assert all(issubclass(dim, Dimension) for dim in params)

        return super(Index, cls).__class_getitem__(Tuple[params])
...
def is_index_like(val):
    return isinstance(val, (Index, int)) or val == math.inf or val == -math.inf
...
class ProductSet(CartesianSet):
    ...
    def __contains__(self, arg: Sequence[Integer]):
        assert all(is_index_like(i) for i in arg)
...
class MaskedCartesianSet(CartesianSet):
    ...
    def mask(self, mask):
        return self.set.mask(lambda idx: self.mask_(idx) or mask(idx))

    def __contains__(self, arg: Sequence[Integer]):
        return arg in self.set and not self.mask_(arg)
    ...
    def empty(self) -> bool:
        ...
        return all(mask(idx) for idx in self.set)
